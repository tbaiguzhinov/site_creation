...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for request in new_requests:
            print('creating site...')
            site_id, name, description = create_new_site(token, request)
            update_site(site_id, name, description, token)
            print('creating assessment...')
            assessment_id = create_assessment(token, name, site_id)
            add_assesment_focus(token=token, assessment_id=assessment_id, objectId=304124)
            add_assesment_focus(token=token, assessment_id=assessment_id, objectId=304125)
            print('launching assessment...')
            jobId = confirm_scope(token, assessment_id)
            print(poll_for_status(token, jobId))
